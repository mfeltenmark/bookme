...
import {
  Calendar, CalendarClock, LayoutDashboard, ListChecks, LogOut, Settings, Vote,
} from "lucide-react";
import { cn } from "@/lib/utils";

const navItems = [
  { href: "/admin", label: "Dashboard", icon: LayoutDashboard },
  { href: "/admin/event-types", label: "Event Types", icon: CalendarClock },
        ...
        <header className="flex md:hidden items-center justify-between border-b px-4 py-3">
          ...
          <nav className="flex items-center gap-1">
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={cn("p-2 rounded-md transition-colors", isActive ? "text-primary bg-primary/10" : "text-muted-foreground")}
                  title={item.label}>
                  <item.icon className="h-4 w-4" />
                </Link>
              );
            })}
            <button onClick={handleLogout} className="p-2 rounded-md text-muted-foreground hover:text-foreground" title="Sign out">
